```markdown
| File | Summary |
|---|---| 
| wiki/sources/cv-duynguyen.md | CV of Nguyen Thanh Minh Duy, detailing deep learning research and software engineering experience. |
| wiki/sources/new-minh-duy-profile-2026.md | Minh Duy's current professional and personal profile, detailing his AI specialization and ventures. |
| wiki/sources/minh-duy-profile-2026.md | Minh Duy's current professional and personal profile, detailing his AI specialization and ventures. |
| wiki/sources/bio.md | Profile of Nguyen Thanh Minh Duy. |
| wiki/entities/nguyen-thanh-minh-duy.md | Profile of Nguyen Thanh Minh Duy, a specialist in deep learning and software engineering. |
| wiki/entities/minh-duy-rzy.md | Profile of Minh Duy (Rzy), an AI Engineer specializing in UAV threat quantification and entrepreneurship. |
| wiki/entities/zalo.md | Professional experience at Zalo, focusing on performance engineering and product development. |
| wiki/entities/vng-corporation.md | The corporate entity behind Zalo. |
| wiki/entities/soonchunhyang-university.md | Academic institution where Duy pursued his Master's degree and research. |
| wiki/entities/knowledge-based-systems.md | Journal where Duy published research on intrusion detection systems. |
| wiki/entities/lowkie.md | Profile of Lowkie, a fitness-focused snack initiative. |
| wiki/entities/ho-chi-minh-city.md | Major economic and cultural hub in Southern Vietnam. |
| wiki/entities/ct-group.md | Company interacted with during job search process. |
| wiki/entities/vietnam.md | Country where Minh Duy currently resides and monitors financial markets. |
| wiki/concepts/deep-learning.md | Core methodology used for advanced AI research. |
| wiki/concepts/anomaly-detection.md | Specialized field of detecting deviations in data, particularly in networks. |
| wiki/concepts/few-shot-learning.md | Technique for learning from limited examples. |
| wiki/concepts/open-set-recognition.md | Technique for identifying unknown classes in security systems. |
| wiki/concepts/generative-ai.md | AI capable of creating new content (e.g., LLMs). |
| wiki/concepts/rag-systems.md | Architecture for grounding LLMs with external knowledge. |
| wiki/concepts/multi-modal-ml.md | Processing and understanding multiple data types (text, image, video). |
| wiki/concepts/transformers.md | Foundational neural network architecture using self-attention. |
| wiki/concepts/gnn.md | Network specialized for graph-structured data. |
| wiki/concepts/pytorch.md | Open-source framework for deep learning development. |
| wiki/concepts/fastapi.md | Modern Python framework for building high-performance APIs. |
| wiki/concepts/machine-learning.md | General field of using data to train algorithms for prediction. |
| wiki/concepts/machine-learning-applications.md | Practical implementation of ML techniques to solve real-world problems. |
| wiki/concepts/uavs.md | Aircraft operating without a human pilot, central to Minh Duy's current research. |
| wiki/concepts/sora-framework.md | Structured methodology for assessing operational risks, particularly involving Unmanned Aerial Vehicles (UAVs). |
| wiki/concepts/ai-engineer.md | Professional role focused on designing and implementing practical AI solutions. |
| wiki/concepts/ai-scientist.md | Professional role focused on theoretical AI research and development. |
| wiki/concepts/entrepreneurship.md | The process of creating and managing a new business venture. |
| wiki/concepts/fitness.md | State of being physically healthy, related to bodybuilding and nutrition. |
| wiki/concepts/vietnamese-stock-market.md | Financial market of Vietnam, monitored by Minh Duy. |
```